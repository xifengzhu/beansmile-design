...
const RULES = [
  { id: "wcag-1.4.3-contrast-minimum", platforms: ["web", "mobile_web"] },
  { id: "web-form-label-association", platforms: ["web", "mobile_web"] },
  { id: "craft-spacing-rhythm", platforms: ["web", "mobile_web", "ios", "android", "mini_program"] },
  { id: "hig-navigation-patterns", platforms: ["ios"] },
...
test("目标平台全部适用规则逐条覆盖 → 通过；平台外规则不要求", () => {
  ...
  assert.deepEqual(semanticIssuesStandards(d, ["web"], RULES), []);
});

test("空 findings + 空/缺覆盖 → 报覆盖缺口（形式化 pass 被拒）", () => {
  const issues = semanticIssuesStandards(doc([cover("craft-spacing-rhythm")]), ["web"], RULES);
  assert.ok(issues.some((s) => s.includes("覆盖缺口") && s.includes("缺 2 条")));
});

test("iOS 项目要求 hig 规则；覆盖引用不存在的规则/重复条目 → 拒绝", () => {
  const missing = semanticIssuesStandards(doc([cover("craft-spacing-rhythm")]), ["ios"], RULES);
  ...
  const issues = semanticIssuesStandards(dup, [], RULES);
  assert.ok(issues.some((s) => s.includes("重复条目")));
  assert.ok(issues.some((s) => s.includes("不存在的规则: no-such-rule")));
});
...
test("Web 规则用 intent_only → 拒绝；原生规则 intent_only 合法", () => {
  ...
  const issues = semanticIssuesStandards(doc(full), ["web", "ios"], RULES);
...
test("覆盖证据过短 → 拒绝", () => {
  const d = doc([cover("craft-spacing-rhythm", "pass", { evidence: "符合" })]);
  assert.ok(semanticIssuesStandards(d, [], RULES).some((s) => s.includes("证据过短")));
});

test("result=fail 无对应 finding → 拒绝；有对应 blocker/warning → 通过", () => {
  const noLink = doc([cover("craft-spacing-rhythm", "fail")]);
  assert.ok(semanticIssuesStandards(noLink, [], RULES).some((s) => s.includes("无对应 blocker/warning")));
  const linked = {
    ...doc([cover("craft-spacing-rhythm", "fail")]),
    verdict: "fail",
  ...
  assert.deepEqual(semanticIssuesStandards(linked, [], RULES), []);
});

test("行业规则只在 industry 匹配时进入适用集（含 saas_b2b → industry-saas-b2b.yaml 归一）", () => {
  ...
  assert.deepEqual(semanticIssuesStandards(doc(base), ["web"], RULES), []);
  // industry=ecommerce：电商包进入适用集，未覆盖即缺口；saas 包仍不适用
  const issues = semanticIssuesStandards(doc(base), ["web"], RULES, "ecommerce");
  assert.ok(issues.some((s) => s.includes("ecom-guest-checkout-option")));
  assert.ok(!issues.some((s) => s.includes("saas-destructive-confirm")));
  // slug 下划线归一：saas_b2b 匹配 industry-saas-b2b.yaml
  const saas = semanticIssuesStandards(doc(base), ["web"], RULES, "saas_b2b");
  assert.ok(saas.some((s) => s.includes("saas-destructive-confirm")));
  assert.deepEqual(semanticIssuesStandards(doc([...base, cover("saas-destructive-confirm")]), ["web"], RULES, "saas_b2b"), []);
});

test("blocker finding 的规则在矩阵里写 pass → 自相矛盾被拒", () => {
  const d = doc([cover("craft-spacing-rhythm", "pass")],
  ...
  assert.ok(semanticIssuesStandards(d, [], RULES).some((s) => s.includes("自相矛盾")));
});
