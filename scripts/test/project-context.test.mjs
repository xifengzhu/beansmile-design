...
const FULL_CTX = {
  project: { name: "x", mode: "professional", task_type: "web_page", platforms: ["web"], reference_system: "none", industry: "general" },
  users: { primary: "访客" },
  goals: ["g1"],
  brand: { tone: "冷静" },
  constraints: ["c1"],
...
test("对抗：全部 5 个流程 Skill 的投影键集 ⊆ reads 展开（confirmations/decisions 等绝不泄漏）", () => {
  const { manifests } = loadManifests();
  assert.equal(manifests.length, 5);
  for (const m of manifests) {
    const view = projectContext(FULL_CTX, m.reads);
    const allowedTop = new Set(m.reads.map((p) => p.split(".")[0]));
    for (const top of Object.keys(view)) {
      assert.ok(allowedTop.has(top), `${m.skill} 投影泄漏了未声明字段: ${top}`);
      const seconds = m.reads.filter((p) => p.startsWith(`${top}.`)).map((p) => p.split(".")[1]);
      if (seconds.length) {
    ...
    if (!m.reads.some((p) => p.startsWith("confirmations"))) assert.ok(!("confirmations" in view));
    if (!m.reads.some((p) => p.startsWith("exceptions"))) assert.ok(!("exceptions" in view));
  }
});
