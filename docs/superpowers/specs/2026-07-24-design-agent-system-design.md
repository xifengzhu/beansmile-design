...
本文件保留 package format 1-2 的基础设计。`package_format_version: 3` 起，Design.md 前置契约、开发交接文档与可编辑 PPTX 由版本化扩展 [2026-07-26-delivery-artifacts-design.md](2026-07-26-delivery-artifacts-design.md) 定义；扩展不改变本文件的 canonical lifecycle stage。PDF 仍不进入当前核心流程。
...
- package format 1-2 不承担 PPT/PDF 的叙事与分页设计；format 3 的 PPTX 以版本化扩展为准，PDF 仍是非目标。
...
3. 真实用户测试记录与研究资料适配器。
4. PDF 报告与长文档输出 Skill。

PPT 输出已由 package format 3 的交付扩展实现；它独立定义叙事、可编辑对象、来源、逐页渲染与 Director QA，而不是复用产品界面的页面结构。PDF 扩展仍需另行定义分页与打印检查。
...
### 27.11 复审修正二（第二轮外部复审，3 P1 + 1 P2：核心任务证明与验收 fail-closed）

第二轮外部复审（针对 65acc60）证明三个对抗包仍被误判通过，共性根因：**"任务可用"与"证据在场"的默认方向是开的**——信号存在才检查，缺失即放行。本节逐条反转默认：

- **场景执行期错误捕获（checks_version 7，P1）**：此前场景执行的新 context 没有注册 pageerror/console/requestfailed 监听——交互抛异常仍报"控制台错误 0、核心任务全过"。修正：场景 context 与静态巡检同等捕获三类错误；执行期出错既让该场景 fail，也计入 `console_errors` 阻断信号；步骤跑完后留 120ms 异步错误落地窗口。
- **空断言场景判废（P1）**：`click 任意元素 + expect 静态恒真元素`可把任务证明退化成页面加载测试。三层封堵：① 静态门——`flow` 引用须 ≥4 个含实义字符（"#" 等碎片能匹配任何 Markdown）；② 静态门——`expect_*` 目标禁裸标签选择器（`main`/`section` 在静态页恒真），必须带 `#id`/`.class`/`[attr]`；③ 运行时门——browser-check 在执行任何交互**之前**对全部 `expect_*` 做初始态求值（不等待、立即判定），若全部已成立则场景判废：断言必须是交互建立的状态变化。
- **results.json fail-closed（P1）**：验收此前只对"存在的字段"做信号判断——`focus_visible_ratio`/`reflow_ok`/`zoom_ok`/`task_flows` 缺失静默通过，伪造最小文档（checks_version+同源哈希+键盘比例）可绕过大量浏览器证据。新增 `scripts/lib/results-check.mjs`（纯函数）：当前版本产物的**每个**证据字段必须在场、类型正确、且与交付物对账——`pages[]` 与原型页面清单一一对应（漏页=有页面没被巡检）、截图数=页面数×2、`cls`/`edge_insets` 逐页在场、`task_flows.total` 等于**现算**的 scenarios.json 场景数（scenarios.json 在 prototype/ 内被同源指纹锚定，伪造须同时改哈希）且全过。配套奇偶校验测试：browser-check 真产物必须恒过该校验，门收紧不得误伤。
- **视觉证据多样性按内容判定（P2）**：多样性此前按路径去重——两个文件名指向相同字节、八维交替引用即绕过。改按 sha256 判定，两个分支封两种造假形态：盘上文件 ≥2 但内容仅 1 份（库本身注水）→"证据退化"拒；盘上确有 ≥2 份不同内容但八维引用内容 <2 份（换名引同一张）→"未逐维看图"拒。单图包降级为哈希匹配的既有豁免保留。
